...
namespace engine::render {
    ...
    Result<Display> createDisplay(dxgi::Output output) {
        ...
        auto monitor = desc.Monitor;

        DWORD count = 0;
        GetNumberOfPhysicalMonitorsFromHMONITOR(monitor, &count);

        std::vector<Display::Physical> physicals(count);
        GetPhysicalMonitorsFromHMONITOR(monitor, count, physicals.data());

        return pass(Display { output, desc, physicals });
    }

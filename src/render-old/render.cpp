...
namespace engine::render {
    HRESULT Context::createCore() {
        ...
        if (HRESULT hr = D3D12CreateDevice(adapter.get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&device)); FAILED(hr)) {
            log::render->fatal("failed to create device\n{}", to_string(hr));
            return hr;
        }

        {
            ...
            if (info.valid()) {
                ...
                DWORD cookie = 0;
                if (HRESULT hr = it->RegisterMessageCallback(callback, D3D12_MESSAGE_CALLBACK_FLAG_NONE, log::render, &cookie); FAILED(hr)) {
                    log::render->fatal("failed to register message callback\n{}", to_string(hr));
                    return hr;
                }
